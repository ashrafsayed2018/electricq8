...
<div dir="{{ $isAr ? 'rtl' : 'ltr' }}">
    ...
    <section class="eq8-page-hero">
        <div class="eq8-page-hero__inner">
            ...
            <div class="eq8-post-meta-bar" style="justify-content:center;margin-top:16px">
                <span class="eq8-post-meta-bar__item">
                    ...
                    {{ number_format($cluster->views) }} {{ $isAr ? 'مشاهدة' : 'views' }}
                </span>
                <span class="eq8-post-meta-bar__item">
                    ...
                    <span dir="ltr">{{ $cluster->updated_at->format('d M Y') }}</span>
                </span>
            </div>
        </div>
    </section>
    ...
    <section style="padding:48px 0 {{ $content ? '0' : '48px' }};background:var(--bg)">
        <div class="container mx-auto px-4" style="max-width:960px">
            <img src="{{ $cluster->image_url }}" alt="{{ $title }}" class="eq8-cluster-image">
        </div>
    </section>
    @endif
...
.eq8-cluster-image { width:100%; height:auto; max-height:520px; object-fit:contain; border-radius:18px; border:1px solid var(--border); display:block; margin:0 auto; }
