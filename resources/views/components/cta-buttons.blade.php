...
<div {{ $attributes->class(['eq8-hbtns', 'eq8-hbtns--' . $size]) }}>
    <a href="{{ $waUrl }}" target="_blank" rel="noopener" class="eq8-hbtn eq8-hbtn--wa">
        <span class="eq8-hbtn__icon-wrap eq8-hbtn__icon-wrap--wa">
            <svg width="{{ $size === 'lg' ? 24 : 18 }}" height="{{ $size === 'lg' ? 24 : 18 }}" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
    ...
    <a href="tel:{{ $phone }}" class="eq8-hbtn eq8-hbtn--call">
        <span class="eq8-hbtn__icon-wrap eq8-hbtn__icon-wrap--call">
            <svg width="{{ $size === 'lg' ? 20 : 16 }}" height="{{ $size === 'lg' ? 20 : 16 }}" fill="none" stroke="currentColor" stroke-width="2.2"
...
[dir="rtl"] .eq8-hbtn { padding: 6px 6px 6px 22px; }
.eq8-hbtn:hover { transform: translateY(-2px); box-shadow: 0 10px 24px rgba(0,0,0,.28); }
...
.eq8-hbtn--outline { background: transparent; color: #fff; border: 2px solid rgba(255,255,255,.4); padding: 12px 22px; }
.eq8-hbtn--outline:hover { background: rgba(255,255,255,.1); border-color: #fff; }

/* Large variant — used in primary page heroes */
.eq8-hbtns--lg .eq8-hbtn { font-size: .95rem; padding: 5px 22px 5px 5px; }
[dir="rtl"] .eq8-hbtns--lg .eq8-hbtn { padding: 5px 5px 5px 22px; }
.eq8-hbtns--lg .eq8-hbtn__icon-wrap { width: 46px; height: 46px; }

@media (max-width: 480px) {
    .eq8-hbtns { flex-direction: column; align-items: stretch; }
    .eq8-hbtn { justify-content: center; }
}
</style>
